...
var GameSetting=(function(){
	...
	GameSetting.COLOR_2="#eee4da";
	GameSetting.COLOR_4="#ede0c8";
	GameSetting.COLOR_8="#f2b179";
	GameSetting.COLOR_16="#f59563";
	GameSetting.COLOR_32="#f67c5f";
	GameSetting.COLOR_64="#f65e3b";
	GameSetting.COLOR_128="#edcf72";
	GameSetting.COLOR_256="#edcc61";
	GameSetting.COLOR_512="#edc850";
	GameSetting.COLOR_1024="#edc53f";
	GameSetting.COLOR_2048="#edc22e";
	GameSetting.CUBE_CLW=90;
	__static(GameSetting,
	['NUM_ARR',function(){return this.NUM_ARR=[2,4,8,16,32,64,128,256,512,1024,2048];},'COLOR_ARR',function(){return this.COLOR_ARR=["#CDC1B3","#eee4da","#ede0c8","#f2b179","#f59563","#f67c5f","#f65e3b","#edcf72","#edcc61","#edc850","#edc53f","#edc22e"];}
	]);
	return GameSetting;
})()
...
var Main=(function(){
	function Main(){
		...
		Laya.loader.load("res/atlas/comp.atlas",Handler.create(this,this.onComplete));
	}
	...
	__proto.onComplete=function(){
		...
		this.game.x=(Laya.stage.width-400)*0.5;
		this.game.y=(Laya.stage.height-400)*0.5;
		this.game.on("game_over",this,this.onGameOver);
		this.game.on("game_win",this,this.onGameWin);
		this.mainUI=new MainUI();
		Laya.stage.addChild(this.mainUI);
		this.mainUI.begin_btn.on("click",this,this.onBegin);
	}

	__proto.onBegin=function(e){
		this.mainUI.visible=false;
		this.game.gameInit();
	}

	__proto.onGameOver=function(e){
		this.mainUI.visible=true;
		this.mainUI.begin_btn.label="重新开始"
	}

	__proto.onGameWin=function(e){
		this.mainUI.visible=true;
		this.mainUI.begin_btn.label="恭喜 恭喜"
	}
...
*@author HappyKing
*/
//class BlackGround extends laya.display.Sprite
var BlackGround=(function(_super){
	function BlackGround(){
		/**小方块之间的间隔*/
		this.interval=NaN;
		BlackGround.__super.call(this);
		this.clw=90;
		this.interval=100-this.clw;
		console.log(this.interval)
		this.draw();
	}

	__class(BlackGround,'BlackGround',_super);
	var __proto=BlackGround.prototype;
	/**绘制背景*/
	__proto.draw=function(){
		this.graphics.drawRect(-this.interval *0.5,-this.interval *0.5,100 *4+this.interval,100 *4+this.interval,"#BDADA0");
		var i=0;
		var j=0;
		for (i=0;i < 4;i++){
			for (j=0;j < 4;j++){
				this.graphics.drawRect(100 *i+this.interval *0.5 ,100 *j+this.interval *0.5,this.clw,this.clw,"#CDC1B3");
			}
		}
	}
...
*@author HappyKing
*/
//class Cube extends laya.display.Sprite
var Cube=(function(_super){
	function Cube(stpe){
		...
		this.stpe=NaN;
		this.tween=null;
		Cube.__super.call(this);
		(stpe===void 0)&& (stpe=1);
		this.clw=90;
		this.stpe=stpe;
		this.num=new Text();
		this.num.fontSize=40;
		this.num.color="#fff";
		this.num.bold=true;
		this.num.width=this.clw;
		this.num.height=this.clw;
		this.num.valign="middle";
		this.num.align="center"
		this.num.pos(-this.clw / 2,-this.clw / 2);
		this.addChild(this.num);
		this.draw();
	}

	__class(Cube,'Cube',_super);
	var __proto=Cube.prototype;
	/**绘制方块*/
	__proto.draw=function(stpe){
		(stpe===void 0)&& (stpe=0);
		if (stpe==0){
			this.num.text="";
			if (this.tween){
			...
			this.num.text=GameSetting.NUM_ARR[stpe-1];
		}
		this.graphics.clear();
		this.graphics.drawRect(-this.clw / 2,-this.clw / 2,this.clw,this.clw,GameSetting.COLOR_ARR[stpe]);
	}

	/**入场动画*/
	__proto.animal_in=function(){
		this.scaleX=this.scaleY=1;
		this.tween=Tween.from(this,{scaleX:0,scaleY:0},300,Ease.backOut);
	}
...
var GameMain=(function(_super){
	...
	__proto.onMouseUp=function(e){
		...
		var xxNum=e.stageX-this.tampX;
		var yyNum=e.stageY-this.tampY;
		if (Math.abs(xxNum)> Math.abs(yyNum)){
			if (xxNum < 0){
				this.onStpe(3);
			}
			else{
		...
		else if(Math.abs(xxNum)< Math.abs(yyNum)){
			if (yyNum < 0){
				this.onStpe(1);
			}
			else{
	...
	/**
	*移动一步
	*@param dir 方向 1:上 2:下 3:左 4右
	*/
	__proto.onStpe=function(dir){
		(dir===void 0)&& (dir=1);
		var bool1=false;
		var bool2=false;
		var bool3=false;
		switch(dir){
			case 1:
				bool1=this.moveZero(1);
				bool2=this.congener(1);
				bool3=this.moveZero(1);
				break ;
			case 2:
				bool1=this.moveZero(2);
				bool2=this.congener(2);
				bool3=this.moveZero(2);
				break ;
			case 3:
				bool1=this.moveZero(3);
				bool2=this.congener(3);
				bool3=this.moveZero(3);
				break ;
			case 4:
				bool1=this.moveZero(4);
				bool2=this.congener(4);
				bool3=this.moveZero(4);
				break ;
			}
		this.changeData();
	...
	/**添加新数据*/
	__proto.addNumData=function(){
		var xx=GameMain.randomInt(0,3);
		var yy=GameMain.randomInt(0,3);
		while (this.dataArr[yy][xx] !=0){
			xx=GameMain.randomInt(0,3);
			yy=GameMain.randomInt(0,3);
		};
		var stpe=GameMain.randomInt(1,2);
		this.cubeArr[yy *4+xx].draw(stpe);
		this.cubeArr[yy *4+xx].animal_in();
		this.dataArr[yy][xx]=stpe;
	}
	...
	__proto.testOver=function(){
		for (var i=0;i < 4;i++){
			for (var j=0;j < 4;j++){
				...
				if (j+1 < 4){
					if (this.dataArr[i][j]==this.dataArr[i][j+1]){
						return false;
					}
				}
				if (i+1 < 4){
					if (this.dataArr[i][j]==this.dataArr[i+1][j]){
						return false;
					}
				}
			}
	...
	*false 未成功 true 成功
	*/
	__proto.testWin=function(){
		var i=0;
		var j=0;
		for (i=0;i < 4;i++){
			for (j=0;j < 4;j++){
				if (this.dataArr[i][j] >=11){
					return true;
				}
			}
		}
	...
	*归零 交换0的位置
	*@param dir 方向 1:上 2:下 3:左 4右
	*@return false 没有移动过 true 有cube移动过
	*/
	__proto.moveZero=function(dir){
		(dir===void 0)&& (dir=1);
		var i=0;
		var j=0;
		var k=0;
		...
		switch(dir){
			case 1:
				for (j=0;j < 4;j++){
					for (i=1;i <4;i++){
						...
						if (this.dataArr[i][j] !=0){
							for (k=i-1;k >=0;k--){
								if (this.dataArr[k][j]==0){
									zeroNum++;
									bool=true;
								}
							};
							var tamp=this.dataArr[i][j]
							this.dataArr[i][j]=0;
							this.dataArr[i-zeroNum][j]=tamp;
						}
					}
				}
				break ;
			...
			case 3:
				for (j=0;j < 4;j++){
					for (i=1;i <4;i++){
						zeroNum=0;
						if (this.dataArr[j][i] !=0){
							for (k=i-1;k >=0;k--){
								if (this.dataArr[j][k]==0){
									zeroNum++;
									bool=true;
								}
							};
							var tamp=this.dataArr[j][i]
							this.dataArr[j][i]=0;
							this.dataArr[j][i-zeroNum]=tamp;
						}
					}
				}
				break ;
	...
	*合并同类
	*@param dir 方向 1:上 2:下 3:左 4右
	*@return false 没有合并的cube true 有合并的cube
	*/
	__proto.congener=function(dir){
		(dir===void 0)&& (dir=1);
		var i=0;
		var j=0;
		var bool=false;
		switch(dir){
			case 1:
				for (j=0;j < 4;j++){
					for (i=0;i <3;i++){
						if (this.dataArr[i][j]==this.dataArr[i+1][j] && this.dataArr[i][j] !=0){
							this.dataArr[i][j]++;
							this.dataArr[i+1][j]=0;
							this.cubeArr[i *4+j].draw(this.dataArr[i][j]);
							this.cubeArr[i *4+j].animal_in();
							bool=true;
						}
					}
				}
			...
			case 3:
				for (j=0;j < 4;j++){
					for (i=0;i <3;i++){
						if (this.dataArr[j][i]==this.dataArr[j][i+1] && this.dataArr[j][i] !=0){
							this.dataArr[j][i]++;
							this.dataArr[j][i+1]=0;
							this.cubeArr[j *4+i].draw(this.dataArr[j][i]);
							this.cubeArr[j *4+i].animal_in();
							bool=true;
						}
					}
				}
			...
			case 4:
				for (j=0;j < 4;j++){
					for (i=3;i >0;i--){
						if (this.dataArr[j][i]==this.dataArr[j][i-1] && this.dataArr[j][i] !=0){
							console.log("前:"+this.dataArr[j][i])
							this.dataArr[j][i]++;
							this.dataArr[j][i-1]=0;
							console.log("后:"+this.dataArr[j][i])
							this.cubeArr[j *4+i].draw(this.dataArr[j][i]);
							this.cubeArr[j *4+i].animal_in();
							bool=true;
						}
	...
	GameMain.randomInt=function(min,max){
		return parseInt(Math.random()*(max-min+1)+min);
	}

	GameMain.GAME_OVER="game_over";
	GameMain.GAME_WIN="game_win";
	return GameMain;
})(Sprite)
...
var MainUIUI=(function(_super){
	...
	__static(MainUIUI,
	['uiView',function(){return this.uiView={"type":"View","props":{"width":640,"height":1136},"child":[{"type":"Image","props":{"y":0,"x":0,"width":640,"skin":"comp/blank.png","height":1136}},{"type":"Button","props":{"y":525,"x":171,"width":298,"var":"begin_btn","skin":"comp/button.png","sizeGrid":"5,6,5,4","labelSize":50,"label":"开始游戏","height":86}}]};}
	]);
	return MainUIUI;
})(View)
