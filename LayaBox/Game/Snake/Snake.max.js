...
var Main=(function(){
	...
	__proto.onComplete=function(){
		...
		back.x=(Laya.stage.width-500)*0.5;
		back.y=(Laya.stage.height-500)*0.5;
		this.game=new GameMain();
		Laya.stage.addChild(this.game);
		this.game.x=(Laya.stage.width-500)*0.5;
		this.game.y=(Laya.stage.height-500)*0.5;
		this.game.on("SourceChange",this,this.onSourceChange);
		this.game.on("GameOver",this,this.onGameOver);
		this.mainUI=new MainUI();
		Laya.stage.addChild(this.mainUI);
		this.mainUI.begin_btn.on("click",this,this.onBegin);
		this.mainUI.pasued_btn.visible=false;
		this.mainUI.pasued_btn.on("click",this,this.onPaused);
	}

	/**游戏开始*/
	__proto.onBegin=function(){
		this.mainUI.bg_image.visible=false;
		this.mainUI.begin_btn.visible=false;
		this.mainUI.pasued_btn.visible=true;
		this.mainUI.source_txt.text=0;
		this.game.gameInit();
	}

	/**游戏结束*/
	__proto.onGameOver=function(){
		this.mainUI.bg_image.visible=true;
		this.mainUI.begin_btn.visible=true;
		this.mainUI.pasued_btn.visible=false;
		this.mainUI.begin_btn.label="重新开始";
	}

	/**分数改变*/
	__proto.onSourceChange=function(){
		this.mainUI.source_txt.text=this.game.sources.toString();
	}

	__proto.onPaused=function(){
		if (!this.pausedBool){
			this.mainUI.pasued_btn.label="继续游戏";
			this.mainUI.bg_image.visible=true;
			this.game.gamePaused();
		}
		else{
			this.mainUI.pasued_btn.label="暂停游戏"
			this.mainUI.bg_image.visible=false;
			this.game.gameContinue();
		}
		this.pausedBool=!this.pausedBool;
	}
...
var BackGround=(function(_super){
	...
	__proto.draw=function(){
		this.graphics.drawRect(-10,-10,520,520,"#fff","#ccc",10);
		var i=0;
		var j=0;
		for (i=0;i < 25;i+=2){
			for (j=0;j < 25;j+=2){
				this.graphics.drawRect(i *20,j *20,20,20,"#ddd");
			}
		}
		for (i=1;i < 25;i+=2){
			for (j=1;j < 25;j+=2){
...
var GameMain=(function(_super){
	...
	__proto.gameInit=function(){
		this.gameClear();
		this.snakeArr.push(new Point(12,12),new Point(11,12));
		Laya.timer.frameLoop(10,this,this.moveSnake);
		this.randomFoot();
	...
	__proto.onKeyDown=function(e){
		switch(e.keyCode){
			case 38:
				if (this.dir !=2 && this.moveBool){
					this.dir=1;
					this.moveBool=false;
				}
				break ;
			case 40:
				if (this.dir !=1 && this.moveBool){
					this.dir=2;
					this.moveBool=false;
				}
				break ;
			case 37:
				if (this.dir !=4 && this.moveBool){
					this.dir=3;
					this.moveBool=false;
				}
				break ;
			case 39:
				if (this.dir !=3 && this.moveBool){
					this.dir=4;
					this.moveBool=false;
				}
				break ;
	...
	/**
	*蛇 移动一步
	*@param dir 方向 1:上 2:下 3:左 4:右
	*/
	__proto.moveSnake=function(){
		this.moveBool=true;
		switch(this.dir){
			case 1:
				this.snakeArr.splice(0,0,new Point(this.snakeArr[0].x,this.snakeArr[0].y-1));
				break ;
			case 2:
				this.snakeArr.splice(0,0,new Point(this.snakeArr[0].x,this.snakeArr[0].y+1));
				break ;
			case 3:
				this.snakeArr.splice(0,0,new Point(this.snakeArr[0].x-1,this.snakeArr[0].y));
				break ;
			case 4:
				this.snakeArr.splice(0,0,new Point(this.snakeArr[0].x+1,this.snakeArr[0].y));
				break ;
			}
		if (!(this.snakeArr[0].x==this.footX && this.snakeArr[0].y==this.footY)){
			this.snakeArr.pop();
		}
		else{
			this.sources++;
			this.event("SourceChange");
			this.randomFoot();
		}
		this.draw()
		if (this.testOver()){
	...
	__proto.draw=function(){
		...
		this.graphics.clear();
		for (i=1;i < len;i++){
			this.graphics.drawRect(this.snakeArr[i].x *20,this.snakeArr[i].y*20,20,20,"#00f");
		}
		this.graphics.drawRect(this.snakeArr[0].x *20,this.snakeArr[0].y*20,20,20,"#f00");
		this.graphics.drawRect(this.footX*20,this.footY*20,20,20,"#0f0");
	}
	...
	*食物不能出现在蛇的身上
	*/
	__proto.randomFoot=function(){
		this.footX=GameMain.randomInt(0,24);
		this.footY=GameMain.randomInt(0,24);
		while (this.testInSnake(this.footX,this.footY)){
			this.footX=GameMain.randomInt(0,24);
			this.footY=GameMain.randomInt(0,24);
		}
	}
	...
	*@param xx x坐标
	*@param yy y坐标
	*@return false 不在蛇身上 true 在蛇身上
	*/
	__proto.testInSnake=function(xx,yy){
		var len=this.snakeArr.length;
		var i=0;
		for (i=0;i < len;i++){
			if (this.snakeArr[i].x==xx && this.snakeArr[i].y==yy){
				return true;
			}
		}
		return false;
	...
	*1.检测蛇是否撞到墙壁
	*2.检测蛇是否撞到自己
	*@return false 未结束 true 结束
	*/
	__proto.testOver=function(){
		if (this.snakeArr[0].y > 24 || this.snakeArr[0].y < 0 || this.snakeArr[0].x > 24 || this.snakeArr[0].x < 0){
			return true;
		};
		var i=0;
		var len=this.snakeArr.length;
	...
	__proto.gamePaused=function(){
		Laya.timer.clear(this,this.moveSnake);
		Laya.stage.off("keydown",this,this.onKeyDown);
	}
	...
	GameMain.randomInt=function(min,max,stpe){
		(stpe===void 0)&& (stpe=1);
		return parseInt(Math.random()*(max-min+1)+min);
	}

	GameMain.GAME_OVER="GameOver";
	GameMain.SOURCE_CHANGE="SourceChange";
	return GameMain;
})(Sprite)
...
var MainUIUI=(function(_super){
	...
	__static(MainUIUI,
	['uiView',function(){return this.uiView={"type":"View","props":{"width":1136,"height":640},"child":[{"type":"Image","props":{"y":0,"x":0,"width":1136,"var":"bg_image","skin":"comp/blank.png","height":640}},{"type":"Button","props":{"y":290,"x":466,"width":204,"var":"begin_btn","skin":"comp/button.png","sizeGrid":"3,3,3,3","labelSize":35,"label":"开始游戏","height":60}},{"type":"Button","props":{"y":118,"x":912,"width":116,"var":"pasued_btn","skin":"comp/button.png","sizeGrid":"3,3,3,3","labelSize":20,"label":"暂停游戏","height":32}},{"type":"Label","props":{"y":76,"x":922,"text":"得分:","fontSize":20,"color":"#050000"}},{"type":"Label","props":{"y":76,"x":982,"width":38,"var":"source_txt","text":"0","height":20,"fontSize":20,"color":"#050000","align":"center"}}]};}
	]);
	return MainUIUI;
})(View)
