...
*/
//class longLaya.utils.Arrays
var Arrays=(function(){
	function Arrays(){}
	__class(Arrays,'longLaya.utils.Arrays');
	Arrays.addArrs=function(sourceArr,__rest){
		var rest=[];for(var i=1,sz=arguments.length;i<sz;i++)rest.push(arguments[i]);
		if (sourceArr==null){
			sourceArr=[];
		}
		for (var i=0;i < rest.length;i++){
			for (var j=0;j < rest[i].length;j++){
	...
	Arrays.duplicate=function(oArray,bRecursive){
		(bRecursive===void 0)&& (bRecursive=false);
		var oDuplicate;
		if (bRecursive){
			oDuplicate=[];
			for (var i=0;i < oArray.length;i++){
				if (((oArray[i])instanceof Array)){
					oDuplicate[i]=Arrays.duplicate(oArray[i]);
				}
				else{
	...
	Arrays.equals=function(aArrayA,aArrayB,bNotOrdered,bRecursive){
		(bNotOrdered===void 0)&& (bNotOrdered=false);
		(bRecursive===void 0)&& (bRecursive=false);
		if (aArrayA.length !=aArrayB.length){
			return false;
		};
		var aArrayACopy=aArrayA.concat();
		var aArrayBCopy=aArrayB.concat();
		if (bNotOrdered){
			aArrayACopy.sort();
			aArrayBCopy.sort();
		}
		for (var i=0;i < aArrayACopy.length;i++){
			if (((aArrayACopy[i])instanceof Array)&& bRecursive){
				if (! Arrays.equals(aArrayACopy[i],aArrayBCopy[i],bNotOrdered,bRecursive)){
					return false;
				}
			}
			else if ((typeof (aArrayACopy[i])=='object')&& bRecursive){
				if (! Arrays.objectEquals(aArrayACopy[i],aArrayBCopy[i])){
					return false;
				}
			}
	...
	Arrays.max=function(aArray){
		var aCopy=aArray.concat();
		aCopy.sort(Array.NUMERIC);
		var nMaximum=Number(aCopy.pop());
		return nMaximum;
	}

	Arrays.min=function(aArray){
		var aCopy=aArray.concat();
		aCopy.sort(Array.NUMERIC);
		var nMinimum=Number(aCopy.shift());
		return nMinimum;
	}

	Arrays.sum=function(aArray){
		var nSum=0;
	...
	Arrays.average=function(aArray){
		for (var i=0;i < aArray.length;i++){
			console.log(typeof aArray[i])
			if (typeof aArray[i] !="number"){
				console.log("数组中含有非数值型成员")
				return 0
			}
		}
		return Arrays.sum(aArray)/ aArray.length;
	}

	Arrays.randomize=function(aArray){
		var aCopy=aArray.concat();
		var aRandomized=[];
		var oElement;
		var nRandom=NaN;
	...
	Arrays.switchElements=function(aArray,nIndexA,nIndexB){
		var oElementA=aArray[nIndexA];
		var oElementB=aArray[nIndexB];
		aArray.splice(nIndexA,1,oElementB);
		aArray.splice(nIndexB,1,oElementA);
	}

	Arrays.findMatchIndices=function(aArray,oElement){
		var aIndices=[];
		var nIndex=aArray.indexOf(oElement);
		while (nIndex !=-1){
	...
	Arrays.toListTrace=function(oArray){
		var sIndent="";
		var sOutput="";
	...
	Arrays.getRandomFromArr=function(arr,isDelete){
		(isDelete===void 0)&& (isDelete=true);
		if (arr.length==0){
			console.log("数组为空")
			return null
		};
		var ram=Maths.randomInt(0,arr.length-1);
		var num=arr[ram];
		if (isDelete){
			arr.splice(ram,1);
		}
	...
	Arrays.objectEquals=function(oInstanceA,oInstanceB){
		for (var sItem in oInstanceA){
			if ((typeof (oInstanceA[sItem])=='object')){
				if (! Arrays.objectEquals(oInstanceA[sItem],oInstanceB[sItem])){
					return false;
				}
			}
			else{
...
*@time 2017/1/2 12:54
*/
//class longLaya.utils.Colors
var Colors=(function(){
	function Colors(){}
	__class(Colors,'longLaya.utils.Colors');
	Colors.rgbSplit=function(color){
		if (color.length==4){
			color="#"+color.charAt(1)+color.charAt(1)+color.charAt(2)+color.charAt(2)+color.charAt(3)+color.charAt(3)
		};
		var rgb_R=Color.create("#"+color.toString().substr(1,2)).numColor;
		var rgb_G=Color.create("#"+color.toString().substr(3,2)).numColor;
		var rgb_B=Color.create("#"+color.toString().substr(5,2)).numColor;
		var obj={
			R:rgb_R,
			G:rgb_G,
			B:rgb_B
	...
	Colors.rgbMerge=function(R,G,B){
		var rgb_r=R.toString(16)
		var rgb_g=G.toString(16)
		var rgb_b=B.toString(16)
		if (rgb_r.length==1){
			rgb_r="0"+rgb_r;
		}
		if (rgb_g.length==1){
	...
	Colors.rgb2hsb=function(color){
		var rgb=Colors.rgbSplit(color);
		var rgb_R=rgb.R;
		var rgb_G=rgb.G;
		var rgb_B=rgb.B;
		var arr=[rgb_R,rgb_G,rgb_B];
		var max=Arrays.max(arr);
		var min=Arrays.min(arr);
		var hsb_H=NaN;
		var hsb_S=NaN;
		var hsb_B=NaN;
		if (min==max){
	...
	Colors.hsb2rgb=function(hsb_H,hsb_S,hsb_B){
		var rgb_R=NaN;
		var rgb_G=NaN;
		var rgb_B=NaN;
		hsb_H=(hsb_H >=360)? 0 :hsb_H;
		if (hsb_S==0){
			...
			var i=Math.floor(hsb_H / 60)% 6;
			var f=hsb_H / 60-i;
			var p=hsb_B *(1-hsb_S);
			var q=hsb_B *(1-hsb_S *f);
			var t=hsb_B *(1-hsb_S *(1-f));
			switch (i){
				case 0 :
...
var Maths=(function(){
	...
	Maths.asDegrees=function(radians){
		return radians *Maths.RADTODEG;
	}

	Maths.asRadians=function(degrees){
		return degrees *Maths.DEGTORAD;
	}

	Maths.round=function(nNumber,nRoundToInterval){
		(nRoundToInterval===void 0)&& (nRoundToInterval=1);
		return Math.round(nNumber / nRoundToInterval)/ (1/nRoundToInterval);
	}

	Maths.floor=function(nNumber,nRoundToInterval){
		(nRoundToInterval===void 0)&& (nRoundToInterval=1);
		return Math.floor(nNumber / nRoundToInterval)/ (1/nRoundToInterval);
	}

	Maths.ceil=function(nNumber,nRoundToInterval){
		(nRoundToInterval===void 0)&& (nRoundToInterval=1);
		return Math.ceil(nNumber / nRoundToInterval)/ (1/nRoundToInterval);
	}

	__static(Maths,
	['RADTODEG',function(){return this.RADTODEG=180 / Math.PI;},'DEGTORAD',function(){return this.DEGTORAD=Math.PI / 180;}
	]);
	return Maths;
})()
...
var BackGround=(function(_super){
	function BackGround(){
		...
		var i=0;
		var j=0;
		for (i=0;i < this.xxNum;i++){
			var moArr=[];
			for (j=0;j < this.yyNum;j++){
				var mo=new MovePoint();
				mo.xx=i *120-60;
				mo.yy=j *120-60;
				mo.tampX=i *120-60;
				mo.tampY=j *120-60;
			...
			this.dataArr.push(moArr);
		}
		Laya.timer.loop(50,this,this.draw);
		this.createTriangle();
	}
	...
	__proto.draw=function(){
		...
		var i=0;
		var j=0;
		for (i=0;i < this.xxNum-1;i++){
			for (j=0;j < this.yyNum-1;j++){
				this.triangleArr1[i][j].draw([this.dataArr[i][j].xx,this.dataArr[i][j].yy,this.dataArr[i+1][j].xx,this.dataArr[i+1][j].yy,this.dataArr[i][j+1].xx,this.dataArr[i][j+1].yy])
			}
		}
		for (i=1;i < this.xxNum;i++){
			for (j=1;j < this.yyNum;j++){
				this.triangleArr2[i-1][j-1].draw([this.dataArr[i][j].xx,this.dataArr[i][j].yy,this.dataArr[i-1][j].xx,this.dataArr[i-1][j].yy,this.dataArr[i][j-1].xx,this.dataArr[i][j-1].yy]);
			}
		}
	}

	/**绘制三角形*/
	__proto.createTriangle=function(){
		var i=0;
		var j=0;
		for (i=0;i < this.xxNum-1;i++){
			var upArr=[];
			for (j=0;j < this.yyNum-1;j++){
				...
				triangle1.draw([this.dataArr[i][j].xx,this.dataArr[i][j].yy,this.dataArr[i+1][j].xx,this.dataArr[i+1][j].yy,this.dataArr[i][j+1].xx,this.dataArr[i][j+1].yy]);
				upArr.push(triangle1);
				this.addChild(triangle1);
			}
			this.triangleArr1.push(upArr);
		}
		for (i=1;i < this.xxNum;i++){
			var downArr=[];
			for (j=1;j < this.yyNum;j++){
				var triangle2=new Triangle();
				triangle2.draw([this.dataArr[i][j].xx,this.dataArr[i][j].yy,this.dataArr[i-1][j].xx,this.dataArr[i-1][j].yy,this.dataArr[i][j-1].xx,this.dataArr[i][j-1].yy]);
				downArr.push(triangle2);
				this.addChild(triangle2);
			}
			this.triangleArr2.push(downArr)
		}
	}
...
var Main=(function(_super){
	function Main(){
		...
		var po=new BackGround();
		Laya.stage.addChild(po);
		Laya.stage.graphics.drawRect(340,340,120,120,"#ffcccc","#0")
	}
...
*@author Happyking
*/
//class MovePoint extends laya.display.Sprite
var MovePoint=(function(_super){
	function MovePoint(){
		...
		/**递增值*/
		this.addNum=0;
		MovePoint.__super.call(this);
		this.onMove();
		Laya.timer.loop(50,this,this.onMove);
		this.jiao=Maths.randomInt(0,100);
		this.addNum=(Math.random()*0.01+0.01)*(Maths.randomInt(0,1)==1? 1:-1);
	}

	__class(MovePoint,'MovePoint',_super);
	var __proto=MovePoint.prototype;
	__proto.onMove=function(){
		this.jiao+=this.addNum;
		this.xx=Math.sin(this.jiao)*30+this.tampX;
		this.yy=Math.cos(this.jiao)*10+this.tampY;
		this.x=this.xx;
		this.y=this.yy;
	}
...
*@author Happyking
*/
//class Triangle extends laya.display.Sprite
var Triangle=(function(_super){
	function Triangle(){
		...
		/**角度值*/
		this.jiao=0;
		Triangle.__super.call(this);
		this.draw()
		this.jiao=Maths.randomInt(0,100)
		Laya.timer.loop(50,this,this.onColorChange);
	}

	__class(Triangle,'Triangle',_super);
	var __proto=Triangle.prototype;
	__proto.draw=function(arr){
		(arr===void 0)&& (arr=[]);
		this.graphics.clear();
		this.graphics.drawPoly(0,0 ,arr,this.fillColor,this.fillColor);
	}

	__proto.onColorChange=function(){
		this.h+=0.1
		if (this.h > 360){
			this.h=0;
		}
		this.jiao+=0.05
		this.b=Math.sin(this.jiao)*0.25+0.75;
		var colorArr=Colors.hsb2rgb(this.h,this.s,this.b);
		this.fillColor=Colors.rgbMerge(colorArr[0],colorArr[1],colorArr[2]);
	}
